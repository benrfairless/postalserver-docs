...
* If you are unable or unwilling to grant root access, the database user you create separately needs all permissions on the main database (`postal` by default) and on every mail server database. Mail server databases are created automatically when you add a mail server and are named `{prefix}-server-{id}` (for example `postal-server-1`), where the prefix is set by `message_db.database_name_prefix` (default `postal`). A suitable grant is:

```sql
GRANT ALL PRIVILEGES ON `postal`.* TO 'postal'@'%';
GRANT ALL PRIVILEGES ON `postal-server-%`.* TO 'postal'@'%';
```

Because the second grant uses a wildcard pattern, it also permits Postal to `CREATE` and `DROP` databases matching that pattern.
